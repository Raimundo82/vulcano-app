...
class GetPaidInvoicesUseCase:
    """Return all paid invoices, each annotated with the 12-month average for its account."""
    ...
    def execute(self) -> List[Tuple[Invoice, float]]:
        """
        Return a list of ``(invoice, average)`` pairs for all paid invoices.

        The *average* is the 12-month rolling average of ``total_amount`` for
        the invoice's account.
        """
        invoices = self._repo.get_all_paid()
        result = []
