...
def process_amount_to_pay(amount_str):
    ...
    try:
        ...
        if texto_valor:
            ...
            valor_limpo = (
                texto_valor.strip().replace("€", "").replace(",", ".").replace(" ", "")
            )
    ...
    try:
        ...
        if existing_invoice:
            ...
            SET invoice_type = %s,issue_date = %s, taxpayer_number = %s, account_number = %s, client = %s, address = %s, cvp = %s,
                invoice_period_month = %s, invoice_period_year = %s, total_amount = %s, amount_to_pay = %s, pdffile = %s
            WHERE invoice_number = %s
            """
            cursor.execute(
                update_query,
            ...
            insert_query = """
            INSERT INTO invoices (invoice_type, invoice_number, issue_date, taxpayer_number, account_number, client, address, cvp, invoice_period_month, invoice_period_year, total_amount, amount_to_pay, quitar, pdffile)
